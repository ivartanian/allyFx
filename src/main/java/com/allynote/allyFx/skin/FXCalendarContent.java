...
/**
 * Created by super on 12/16/15.
 */
public class FXCalendarContent extends VBox {

    protected FXCalendarView fxCalendarView;
    ...
    public FXCalendarContent(final FXCalendarView fxCalendarView) {
        this.fxCalendarView = fxCalendarView;
    ...
    void updateGrid() {
        ...
        int nCols = daysPerWeek + (fxCalendarView.isShowWeekNumbers() ? 1 : 0);
        ...
        // Week number column
        if (fxCalendarView.isShowWeekNumbers()) {
            for (int i = 0; i < 6; i++) {
    ...
    void updateDayCells() {
        ...
        for (int i = 0; i < 6 * daysPerWeek; i++) {
            ...
            try {
                ...
                if (date.equals(fxCalendarView.getSelectedDate())) {
    ...
    protected void updateMonthYearPane() {
        ...
        Chronology chrono = fxCalendarView.getChronology();
    ...
    public void goToDate(LocalDate date, boolean focusDayCell) {
        if (isValidDate(fxCalendarView.getChronology(), date)) {
    ...
    public void selectDayCell(DateCell dateCell) {
        fxCalendarView.setSelectedDate(dayCellDate(dateCell));
    }
    ...
    protected void createDayCells() {
        ...
        fxCalendarView.selectedDateProperty().addListener(this::onSelectedDateChanged);
    ...
    private DateCell createDayCell() {
        DateCell cell = null;
        if (fxCalendarView.getDayCellFactory() != null) {
            cell = fxCalendarView.getDayCellFactory().call(fxCalendarView);
        }
        if (cell == null) {
            cell = new DateCell();
        }
    ...
    protected Chronology getPrimaryChronology() {
        return fxCalendarView.getChronology();
    }
...
//            {
                ...
                fxCalendarView.setSelectedDate(pNewValue);
//            }
        }
